...
const form = ref({ name: '', stock_awal: 0, stock_saat_ini: 0, satuan: ""})
...
function openCreate() {
  ...
  editId.value = null
  form.value = { name: '', stock_awal: 0, stock_saat_ini: 0, satuan: "" }
...
//   isOpNaming.value = false; 
//   editId.value = p.id
//   form.value = { name: p.name, stock_awal: p.stock_awal, stock_saat_ini: p.stock_saat_ini, satuan: p.satuan }
//   showModal.value = true
// }
...
<template>
  <DashboardLayout :navLinks="navLinks">
    ...
    <div class="table-wrap">
      ...
      <table v-else>
        <thead><tr><th>No</th><th>Nama barang</th><th>Stok awal</th><th>Stok Saat ini</th><th>Satuan</th><th>Riwayat Opname</th><th>Aksi</th></tr></thead>
        <tbody>
          <tr v-for="(p, i) in barangs" :key="p.id">
            ...
            <td class="actions">
              ...
              <!-- <button class="btn-sm edit" @click="openEdit(p)">Edit</button> -->
    ...
    <div v-if="showModal" class="overlay" @click.self="showModal = false">
      <div class="modal">
        <h2>{{ isOpNaming ? 'Tambah Opname' : (isEditing ? 'Edit Produk' : 'Tambah Produk') }}</h2>
...
.field input, .field select { width: 100%; padding: 9px 12px; background: #13131f; border: 1px solid #363650; border-radius: 8px; color: #e2e8f0; font-size: 14px; outline: none; box-sizing: border-box; }
